...
melted_coord_tbl <- ok_cl_GRange_tbl %>% unnest(bins) %>% dplyr::select(chr,cl,bins,res) %>% mutate(end = as.integer(bins + res_num[res] -1))

cl_bin_melted <- GRanges(seqnames=melted_coord_tbl$chr,
        ranges = IRanges(start=melted_coord_tbl$bins,
                         end=melted_coord_tbl$end
        ))
...
# Bootstrap
nboot<-1e3
obs_count<-melted_coord_tbl %>% 
  mutate(obs = countOverlaps(cl_bin_melted,chr_feature_GRange))%>% 
  group_by(chr,cl) %>% 
  summarise(cl_obs = sum(obs)) %>% 
  dplyr::select(chr,cl,cl_obs)
...
boot_bool<-future_map(1:nboot,function(i){
  ...
  # Compare with obs
  return(as.matrix(melted_coord_tbl %>% 
    mutate(rn = countOverlaps(cl_bin_melted,rnp_Grange)) %>% 
    group_by(chr,cl) %>% 
    summarise(cl_rn = sum(rn)) %>% 
    mutate(p_agg= cl_rn > obs_count$cl_obs) %>% 
    ungroup() %>% 
    dplyr::select(p_agg)))
